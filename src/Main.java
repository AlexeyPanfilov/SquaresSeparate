    ...
    public static void main(String[] args) {
        ...
        for (int i = width; i > 1; i--) {
            if ((width % i == 0) && (length % i == 0)) {
                System.out.println("сторона квадрата = " + i);
                break;
            }
        }
        Instant finish = Instant.now();
        long elapsed = Duration.between(start, finish).toMillis();
        System.out.println("Время выполнения: " + elapsed);
        ...
        System.out.println("общее время выполнения: " + elapsedTotal);
        System.out.println("Рекурсивный метод: " + recursiveSquare(length, width));
        Instant finish3 = Instant.now();
        long elapsedRecursive = Duration.between(start, finish3).toMillis();
        System.out.println("Время после рекурсии: " + elapsedRecursive);
    }
